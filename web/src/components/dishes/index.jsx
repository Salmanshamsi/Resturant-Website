...
const Dishes = ({ open, setOpen, allCategory }) => {
  ...
  const loadCartProducts = useSelector((state) => state?.user?.isLoad);
  ...
  const addToCartItemFunc = (dish, index) => {
    ...
    if (filteredArray?.length > 0) {
      ...
      const addisOrderedBoolean = { ...currentObj, isOrdered: false }
      addisOrderedBoolean.isClicked = true
      isClickedTrue?.splice(index, 1, addisOrderedBoolean)
  ...
  const placeOrder = async () => {
    ...
    if (!userId) {
      ...
      const makePostArray = removeOrderedFood?.map((item, index) => {
        return {
          Image: item?.image,
          quantity: item?.qty,
          price: item?.price,
  ...
  const removeOrderedTotal = product?.filter(({ isOrdered }) => {
    return isOrdered == false
  })
  return (
    <div>
      <ToastContainer
        ...
        theme="light"
      />
      <section className="text-white body-font" id="dishes">
                    ...
                    <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                      ...
                      <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
                        <div className="flex justify-between text-base font-medium text-gray-900">
                          <p>Subtotal</p>
                          <p>{
                            removeOrderedTotal?.reduce((acc, { price }) => {
                              return acc + price
                            }, 0).toFixed(2)
                          }
                          </p>

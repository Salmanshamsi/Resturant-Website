...
const Order = () => {
    ...
    const getAllOrders = async () => {
        setloader(true)
        try {
            const getAllOrder = await getApiMethod("cart/allOrder");
            const getDishes = getAllOrder?.data?.map(({ dish }) => {
                return dish
            })
            if (getDishes?.length > 0) {
                const mergedArray = [].concat(...getDishes);
                setTableListData(mergedArray);
            }
            setloader(false)
        } catch (error) {
            toast.error(`${error?.message}`, {
    ...
    const handleDelete = async (id, userId) => {
        setclicled(true)
        try {
            const deleteDish = await deleteApiMethod("cart/delete", {
                id, userId
            });
    ...
    const handleComplete = async (id, userId) => {
        setclicled(true)
        try {
            const completeDish = await postApiMethod("cart/complete", {
                id, userId
            });
            if (completeDish?.status == 200) {
                toast.success(`${completeDish?.data.message}`, {
                    position: "top-right",
                    autoClose: 1000,
                    hideProgressBar: false,
                    closeOnClick: false,
                            ...
                            <div className="table-card">
                                ...
                                >
                                    ...
                                    <Column
                                        field="quantity"
                                        header="Quantity"
                                        style={{ justifyContent: "center", flex: "0 0 1rem" }}
                                        sortable
                                    ></Column>
                                    <Column
                                        field="userId"
